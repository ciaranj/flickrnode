...
Request.prototype.executeRequest= function(method, arguments, sign_it, result_mapper, callback) {
    ...
    var request= this.getHttpClient().request("GET", 
                          ...
                          {"host": "api.flickr.com"});
    this.listenResponse(request, result_mapper, callback);
    request.close();
};

// Moved respnonse handling to a separate method for reuse.
// This handles the response and then calls the callback.
// It doesn't close the request so make sure to close it after calling this method 
Request.prototype.listenResponse = function(request, result_mapper, callback) {
    ...
    request.addListener('response', function (response) {
        ...
        response.addListener("end", function () {   
            ...
            else if( result.match(/^<\?xml/) ) {   
              //Awesome for 'speed' we'll parse the XML response with reg-ex, prepare for fail, full stream ahead.
              var extractStat= /<rsp stat="(.+?)"/.exec( result );
              if( extractStat && extractStat.length >0 ) {
                var status= extractStat[1];
                if( status == 'fail' ) {
                  var extractFailureReason= /<err code="(.+?)" msg="(.+?)"/.exec( result );
                  require('sys').puts(JSON.stringify(extractFailureReason))
                  if( extractFailureReason && extractFailureReason.length > 1 ) {
                    callback({code: extractFailureReason[1], message: extractFailureReason[2]}, null)
                  } 
                  else {
                    callback({code: -1, message: 'Unable to parse response from server: ' + result}, null);
                  }
                }
                else {
                  //success
                  require('sys').puts('TODO: Return whatever a correct response is!!! success: '+ result)
                }
              }
              else {
                callback({code: -1, message: 'Unable to parse response from server: ' + result}, null);
